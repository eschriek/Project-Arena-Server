...
public class MainDialog extends DialogBox implements
	...
	public MainDialog(final QuestItemDTO itemDTO, boolean readOnlyDialog,
			boolean create) {
		super(false, true);
	...
	private void fillFormFromItem(QuestItemDTO itemDTO) {
	...
	public QuestItemDTO fillItemFromForm(QuestItemDTO itemDTO) {
	...
	public void refresh() {
		items.clear();
		for (Type item : itemDTO.getItems()) {
			if (item instanceof ImageQuestItemDialog.Container) {
				items.addItem("Image");
			} else if (item instanceof QuestionQuestItemDialog.Container) {
				items.addItem("Question");
			} else if (item instanceof VideoQuestItemDialog.Container) {
				items.addItem("Video");
			} else if (item instanceof StoryQuestItemDialog.Container) {
				items.addItem("Story");
			}
		}
	}
	...
	class AddHandler implements ClickHandler {
		...
		public AddHandler(int type, QuestItemDTO itemDTO) {
			this.type = type;
			this.itemDTO = itemDTO;
		}
